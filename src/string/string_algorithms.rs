...
pub fn create_dictionary(text: String) -> HashMap<char, i32> {
...
pub fn find_symbol(text: String, symbol_need_find: char) -> (char, i32) {
    let dictionary = create_dictionary(text);

    if dictionary.contains_key(&symbol_need_find) {
        (symbol_need_find, dictionary[&symbol_need_find])
    } else {
        (symbol_need_find, 0)
    }
}
